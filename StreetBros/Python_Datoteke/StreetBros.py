...
#Z AI sem si pomagal pri skoku igralcev ter pri udarcu igralcev (pozicija in animacija). 
#Vse slike igralcev in specialnih sposobnosti (krogla in streka), vključno s sporočili sem narisal sam.
...
attack_sound = pygame.mixer.Sound("StreetBros\efekti\mixkit-fast-sword-whoosh-2792.wav")
krogla_sound = pygame.mixer.Sound("StreetBros\efekti\pop.mp3")
strela_sound = pygame.mixer.Sound("StreetBros\efekti\pop2.mp3")
#mapi
NightCity = MAP("StreetBros/odzadja/wp5418813-anime-pixel-art-wallpapers.png", length, 0, width, 0, "StreetBros/music/Seek & Destroy (Remastered).mp3")
Forest_Map = MAP("StreetBros/odzadja/forth.jpg", length, 0, width, 0, "StreetBros/music/Master of Puppets (Remastered).mp3")  
...
def krogla_zvok():
    global krogla_sound
    pygame.mixer.Sound.play(krogla_sound)


def strela_zvok():
    global strela_sound
    pygame.mixer.Sound.play(strela_sound)
        ...
        if Player1.udarec_cooldown > 0:
            ...
            if Player1.udarec_frame in [1, 2] and Player1.zadnji_udarec_frame != Player1.udarec_frame:
                if (x1 < Player2.x + 200 and x2 > Player2.x and
                    ...
                    if Player2.blok != True or (Player2.smer == Player1.smer) or Player2.blok == True and Player2.smer == Player1.smer:
